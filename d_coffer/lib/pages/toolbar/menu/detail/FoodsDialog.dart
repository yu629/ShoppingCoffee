...
import 'package:d_coffer/jsonserialize/goods_detail/data.dart';
import 'package:d_coffer/jsonserialize/goods_price/data.dart';
...
import 'package:d_coffer/pages/toolbar/menu/detail/select_row.dart';
...
class FoodsDialog extends StatefulWidget {
  final int id;
  final ShoppingCartModel model;

  FoodsDialog({Key key, this.id, this.model}) : super(key: key);

  @override
  _FoodsDialogState createState() => _FoodsDialogState();
}

class _FoodsDialogState extends State<FoodsDialog> {
  /// 当前选中规格信息
  Map defaultValue = {
    "spec": {
      // typeId: childId,
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () async {
      var res = await Global.req.shop.goodsDetail(id: widget.id);

      Map result = res.data;

      GoodsDetailData goodsDetailData =
          GoodsDetailData.fromJson(result['data']);
  ...
  Future<Map> _getGoodsPrice(Map<String, int> spec,
    ...
    try {
      var res = await Global.req.shop.goodsPrice(
      ...
      child: Column(
        children: <Widget>[
          ...
          Container(
              alignment: Alignment.centerLeft,
              child: Text(
                '${data.basicInfo.name} ${Global.handleGoodsDesc(defaultValue['specName'])}',
      ...
      children: <Widget>[
        Container(
          ...
          child: ClipRRect(
            ...
            child: Image.asset(
              'assets/images/menu/dialog1.jpg',
              height: 150,
              fit: BoxFit.cover,
            ),
          ),
